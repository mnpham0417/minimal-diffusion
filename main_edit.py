...
def main():
    ...
    parser.add_argument("--ft-ckpt", type=str, help="Pretrained model ckpt")
    ...
    model_ft = unets.__dict__[args.arch](
        image_size=metadata.image_size,
        in_channels=metadata.num_channels,
        out_channels=metadata.num_channels,
        num_classes=metadata.num_classes if args.class_cond else None,
    ...
    if args.pretrained_ckpt:
        ...
        if args.delete_keys:
            for k in args.delete_keys:
                ...
                del d[k]
        model_pr.load_state_dict(d, strict=False)
        print(
    ...
    if args.ft_ckpt:
        print(f"Loading pretrained model from {args.ft_ckpt}")
        d = fix_legacy_dict(torch.load(args.ft_ckpt, map_location=args.device))
        dm = model_ft.state_dict()
        if args.delete_keys:
            for k in args.delete_keys:
                print(
                    f"Deleting key {k} becuase its shape in ckpt ({d[k].shape}) doesn't match "
                ...
                del d[k]
        model_ft.load_state_dict(d, strict=False)
        print(
            f"Mismatched keys in ckpt and model: ",
            set(d.keys()) ^ set(dm.keys()),
        )
        print(f"Loaded pretrained model from {args.ft_ckpt}")
