    ...
    with tqdm(total=math.ceil(N / (args.batch_size * num_processes))) as pbar:
        while num_samples < N:
            ...
            gen_images = diffusion.sample_from_reverse_process(
                model, xT, sampling_steps, {"y": y, "soft_point":soft_point}, args.ddim
            )
...
def main():
    ...
    #output file name
    parser.add_argument("--output-file-name", default="score_per_complexity_parameter.txt", type=str)

    parser.add_argument("--complexity-lower", default=1, type=int)
    parser.add_argument("--complexity-upper", default=10, type=int)
    
    parser.add_argument("--random-seed-lower", default=1, type=int)
    parser.add_argument("--random-seed-upper", default=30, type=int)
    ...
    Score_per_complexity_parameter =[]
    For each complexity_parameter n in (1, 10)
    Grid = grid_points(complexity_parameter , d)
    Score_per_seed = []
        For a random_seed in (1,30)
            Genereation_scores_grid = []
            For soft_point in Grid:
                Genereation_scores.append(D(G(random_seed  = random_seed, soft_point = soft_point))
                # G is the generator model, D is the score for the erased class
            Best_attack_per_seed = argmax(Genereation_scores)
            Score_per_seed.append(Genereation_scores[Best_attack_per_seed])
        Score_per_complexity_parameter.append(Average(Score_per_seed))

    Plot((1, 10),Score_per_complexity_parameter)
    ...
    def grid_points(n, d):
        # Generate a grid of points in each dimension centered around 0
        grid_axes = [np.linspace(-1, 1, n, endpoint=False) for _ in range(d)]

        # Create a grid of points
        grid = np.array(np.meshgrid(*grid_axes)).T.reshape(-1, d)

        # Normalize each point to a unit vector
        grid_normalized = grid / np.linalg.norm(grid, axis=1)[:, np.newaxis]

        return grid_normalized

    # Example: Randomize points in a 3x3 grid in 3 dimensions
    n = 10
    dimension = 3
    random_points = randomize_points(n, dimension)

    # 3D Scatter Plot
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(random_points[:, 0], random_points[:, 1], random_points[:, 2], marker='o')

    ax.set_title('Randomized and Normalized Points in a projected grid')
    ...
    def grid_points(n, d):
        ...
        #create grid_random points with same number of points as grid_normalized
        # grid_random = np.random.uniform(-1, 1, size=(n**d, d))
        # grid_random = np.random.normal(0, 1, size=(n**d, d))
        # grid_random_normalized = grid_random / np.linalg.norm(grid_random, axis=1)[:, np.newaxis]
        # grid_normalized = grid_random_normalized
        
        return grid_normalized

    def G(random_seed, soft_point, num_images):
        #set random seed
    ...
    model_cls = resnet18(pretrained=False)
    model_cls.conv1 = nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)  # Adjust for 1 channel
    model_cls.fc = nn.Linear(model_cls.fc.in_features, 10)  # Adjust for 10 classes
    
    model_cls.load_state_dict(torch.load('mnist_resnet18_20.pth'))
    model_cls = model_cls.cuda()

    def D(sampled_images, labels, target_class):
        # print(type(sampled_images), type(labels))
        ...
        with torch.no_grad():
            ...
            count_total = 0
            count_target = 0
            for i, (img_np, label) in enumerate(zip(sampled_images, labels)):
                ...
                #     img_pil.save(save_folder + str(i) + ".png")
    ...
    # num_images = 100
    score_per_complexity_parameter = []
    for complexity_parameter in tqdm(range(args.complexity_lower, args.complexity_upper)):
        print("complexity_parameter: ", complexity_parameter)
        grid = grid_points(complexity_parameter, args.d)
        score_per_seed = []
        for random_seed in range(args.random_seed_lower, args.random_seed_upper):
            print("random_seed: ", random_seed)
            generation_scores = []
            for soft_point in grid:
                sampled_images, labels = G(random_seed=random_seed, soft_point=soft_point, num_images=args.num_images)
                
                count_target = D(sampled_images, labels, args.target_class)
            
                generation_scores.append(count_target)
            best_attack_per_seed = np.argmax(generation_scores)
            score_per_seed.append(generation_scores[best_attack_per_seed])
        score_per_complexity_parameter.append(np.average(score_per_seed))
        #save score_per_complexity_parameter as txt file
        np.savetxt(args.output_file_name, score_per_complexity_parameter)
    
    #save score_per_complexity_parameter as txt file
    np.savetxt(args.output_file_name, score_per_complexity_parameter)
    print(score_per_complexity_parameter)
    '''
    complexity 1, random_seed 1 => 4 points
    complexity 2, random_seed 1 => 8 points        
    '''
