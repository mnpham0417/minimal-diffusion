...
singularity exec --nv \
  ...
  /bin/bash -c 'source /ext3/env.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/diffusion_ft; cd /vast/mp5847/minimal-diffusion/; \
        ...
        --dataset mnist \
        --arch UNet_4 \
        --pretrained-ckpt /vast/mp5847/minimal-diffusion/trained_models_UNet_4_epochs=100_unit_sphere_0_tv=1.7_unconditional/UNet_4_mnist-epoch_100-timesteps_1000-class_condn_True.pt \
        --target-class 0 \
        --output-file-name "resolution_mnist_0_UNet_4_epochs=100_unit_sphere_0_tv=1.7_unconditional_clower=1_cupper=10_rslower=1_rsupper=10_nimages=10.txt" \
        --complexity-lower 1 \
        --complexity-upper 5 \
        --random-seed-lower 1 \
        --random-seed-upper 3 \
