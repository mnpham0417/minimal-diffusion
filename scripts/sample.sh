...
sampling_args="--arch UNet_4 --class-cond --sampling-steps 400 --sampling-only"

CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port 8102 main.py \
    --arch UNet_4 --dataset mnist --batch-size 512 --num-sampled-images 1000 $sampling_args \
    --pretrained-ckpt "/vast/mp5847/minimal-diffusion/trained_models_UNet_4_epochs=100_unit_sphere_0_tv=1.5_random/UNet_4_mnist-epoch_100-timesteps_1000-class_condn_True.pt" \
    --save-dir "/vast/mp5847/minimal-diffusion/trained_models_UNet_4_epochs=100_unit_sphere_0_tv=1.5_random"
