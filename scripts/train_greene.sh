...
singularity exec --nv \
  ...
  /bin/bash -c 'source /ext3/env.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/diffusion_ft; cd /vast/mp5847/minimal-diffusion/; \
        CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port 8106 main.py --arch UNet_1 --dataset mnist --class-cond --epochs 100 --batch-size 512 --sampling-steps 100 --save-dir ./trained_models_UNet_1_unit_sphere/ \'
